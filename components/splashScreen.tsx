...
export const SplashScreen = () => {
    ...
    const startAppAnimation = () => {
        ...
        sizeCircle.value = withTiming(width, { duration: 700, easing: Easing.inOut(Easing.ease) })
        scaleCircle.value = withTiming(3, { duration: 700, easing: Easing.inOut(Easing.ease) }, () => {
    ...
    useEffect(() => {
        setTimeout(() => startAppAnimation(), 2500)
    }, []);

    return (
        <View style={[blStyles.blCnRow, splashScreenStyles.container]}>
            <Animated.View
                style={[splashScreenStyles.startApp, animatedCircle]}
            />
            <Animated.Image
